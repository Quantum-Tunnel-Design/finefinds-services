    ...
    for (const parentInput of parentsData) {
      ...
      if (!existingParent) {
        try {
          ...
          console.log(`Parent user ${parentInput.email} created. AccessToken: ${result.accessToken ? 'Generated' : 'Not Generated'}`);
        } catch (error) {
      ...
      if (!existingVendor) {
        try {
          const result = await authService.vendorSignUp(vendorInput);
           console.log(`Vendor user ${vendorInput.email} created. AccessToken: ${result.accessToken ? 'Generated' : 'Not Generated'}`);
